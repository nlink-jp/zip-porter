...
- ファイル名エンコーディング自動判定: bit 11 → UTF-8 確定。フラグなしは
  **UTF-8 妥当性検査を先に**行い、妥当でない場合のみ CP932 とみなす
  （ASCII のみは両立するのでそのまま）。`--encoding` で強制指定可。
  ※当初案は CP932 先行だったが、UTF-8 の日本語バイト列は CP932 として
  「デコード成功」してしまう（文字化けするだけ）一方、CP932 の日本語が
  UTF-8 として妥当になることはほぼないため、UTF-8 先行が正しい（実装時に
  修正）
