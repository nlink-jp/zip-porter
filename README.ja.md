...
- どちらの操作も実行中はバイト単位で正確なプログレスバー付きの
  動作状況ダイアログを表示します。問題なく完了した場合は OS の通知
  （バナー）で知らせ、OK ボタンなしで Finder 表示まで進みます。
  読むべき内容がある場合（安全でないパスのスキップ、重複名のリネーム等）
  のみ結果ダイアログが残ります。作成中の ZIP は完成まで
  `name.zip.part` という一時名で存在します。ダブルクリックで開いた場合はドロップレットのウィンドウを
